#!/bin/bash
# DefenseClaw shim for pip — inspects package name before executing.
...
REAL_BINARY=$(PATH="$(echo "$PATH" | sed "s|${SHIM_DIR}:||g; s|:${SHIM_DIR}||g")" which pip 2>/dev/null || echo /usr/bin/pip)
...
RESPONSE=$("$CURL_BIN" -s -w "\n%{http_code}" -X POST "http://${API_ADDR}/api/v1/inspect/tool" \
  ...
  -H "X-DefenseClaw-Client: shim/pip/2.0" \
  "${AUTH_HEADER_ARGS[@]+"${AUTH_HEADER_ARGS[@]}"}" \
  --connect-timeout 2 \
  --max-time 5 \
  -d "$(jq -cn --arg tool "pip" --args \
...
if [ "$HTTP_CODE" = "401" ] || [ "$HTTP_CODE" = "403" ]; then
  echo "DefenseClaw: shim auth rejected (HTTP ${HTTP_CODE}) — refusing to exec pip" >&2
  exit 1
fi

ACTION=$(echo "$RESULT" | jq -r '.action // empty' 2>/dev/null) || ACTION=""
if [ -z "${ACTION}" ]; then
  echo "DefenseClaw: shim received unparseable response (HTTP ${HTTP_CODE}) — refusing to exec pip" >&2
  exit 1
fi
