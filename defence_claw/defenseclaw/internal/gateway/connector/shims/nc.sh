...
if [ "$HTTP_CODE" = "401" ] || [ "$HTTP_CODE" = "403" ]; then
  echo "DefenseClaw: shim auth rejected (HTTP ${HTTP_CODE}) — refusing to exec nc" >&2
  exit 1
fi

ACTION=$(echo "$RESULT" | jq -r '.action // empty' 2>/dev/null) || ACTION=""
if [ -z "${ACTION}" ]; then
  echo "DefenseClaw: shim received unparseable response (HTTP ${HTTP_CODE}) — refusing to exec nc" >&2
  exit 1
fi
